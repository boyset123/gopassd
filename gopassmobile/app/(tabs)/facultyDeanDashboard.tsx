  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [slipsResponse, ordersResponse, userResponse] = await Promise.all([
        axios.get<PassSlip[]>(`${API_URL}/pass-slips/dean-pending`, { headers }),
      ...
      // Show orders where the current user is a listed recommender or is acting as the OIC
      // for the next expected recommender (delegated via OIC).
      ...
      <ImageBackground source={headerBgImage} style={styles.screenHeaderBg} imageStyle={styles.screenHeaderImageStyle}>
        <View style={[styles.screenHeaderOverlay, { paddingTop: insets.top + 12 }]}>
          ...
          <View style={styles.screenHeaderInner}>
            ...
            <Text style={styles.screenHeaderSubtitle}>Faculty Dean</Text>
          </View>
                ...
                {selectedItemType === 'slip' && (
                  <>
                    ...
                    <View style={styles.docTitleContainer}>
                      <View />
                      <Text style={styles.docField}>Date: <Text style={styles.docValue}>{formatDate(selectedItem.date)}</Text></Text>
                    </View>
                    ...
                    <View style={styles.docSignatureContainer}>
                      <View style={styles.docSignatureBox}>
                        ...
                        <View style={styles.docSignatureDisplay}>
                          {(selectedItem as PassSlip).signature && <Image source={{ uri: (selectedItem as PassSlip).signature }} style={styles.docSignatureImage} />}
                          <Text style={styles.docSignatureName}>{selectedItem.employee?.name}</Text>
                        </View>
                        <Text style={styles.docSignatureUnderline}>Program Head</Text>
                      </View>
                      <View style={styles.docSignatureBox}>
